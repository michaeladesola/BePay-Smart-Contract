...
function useErc20Info(address) {
  ...
  const totalSupply = useReadContract({ address, abi: ERC20_ABI, functionName: "totalSupply" });
  ...
  return {
    ...
    decimals: Number(dec),
    totalSupply: totalSupply.data ? formatUnits(totalSupply.data, dec) : "—",
  ...
  return (
    <Section title={`${label} (${address.slice(0, 6)}…${address.slice(-4)})`}>
      <div className="grid grid-cols-2 gap-4 text-sm">
        ...
        <div><div className="text-gray-500 dark:text-slate-400">Total Supply</div><div className="font-medium dark:text-slate-100">{info.totalSupply}</div></div>
  ...
  return (
    <Section title="IDO Purchase">
      <div className="space-y-4">
        <div className="flex items-end gap-3">
          <div className="flex-1">
            ...
            <div className="text-xs text-gray-500 dark:text-slate-400 mt-1">
              Estimated Token A (SHR): <span className="font-medium dark:text-slate-100">{estimateA()}</span>
            </div>
            <div className="text-xs text-gray-500 dark:text-slate-400 mt-1">
        ...
        <div className="card p-4 dark:bg-slate-900/80 dark:border-slate-700">
          <div className="flex items-center justify-between text-sm mb-2">
            <span className="text-gray-600 dark:text-slate-300">Your cap usage</span>
            <span className="font-medium dark:text-slate-100">{capText}</span>
          </div>
          <div className="w-full h-3 bg-gray-200 dark:bg-slate-800 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${Math.min(Math.max(capUsedPct, 0), 100)}%` }}
            />
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        <header className="card px-4 py-3 flex items-center justify-between dark:bg-slate-900/80 dark:border-slate-700">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold leading-tight dark:text-slate-100">Sepolia IDO Dashboard</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden sm:block text-xs text-gray-500 dark:text-slate-400">Network: Sepolia (11155111)</div>
          </div>
        </header>
  ...
  return (
    <Layout>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <TokenPanel label="SHR" address={ADDRESSES.SHR} />
        <TokenPanel label="PUSD" address={ADDRESSES.PUSD} />
      </div>
      <Section title="Your Sepolia ETH">
        <div className="text-sm">
          <div className="text-gray-500 dark:text-slate-400">Wallet</div>
          <div className="font-medium dark:text-slate-100">{isConnected ? address : "Not connected"}</div>
          <div className="mt-2 text-gray-500 dark:text-slate-400">Balance</div>
